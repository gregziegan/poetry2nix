{ runCommand, writeText, jq, stdenv, lib, pep425, pep425OSX, pep425Python37 }:

let
  failedTests = lib.debug.runTests {
  ...
  results = writeText "test-pep425-results" (builtins.toJSON failedTests);
in
runCommand "pep425-test"
{ } ''
  results=$(<${lib.escapeShellArg results})
  if [ "$results" != "[]" ]; then
    echo "Failed tests:"
    echo "$results" | ${jq}/bin/jq
    exit 1
  fi
  touch $out
''
